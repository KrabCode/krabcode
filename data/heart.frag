...
void main() {
  ...
  uv.y += .2;
  uv *= rotate2d(-pi*.5);
  float a = s(0., pi, abs(atan(uv.y, uv.x)));
  uv.x += a*1.5-abs(uv.y);
  float d = s(.3,.32, length(uv));
  float pct = 1.-d+a;
  float wavy = clamp(pct*(.5+.5*sin(8.*od-t*10.)),-1., 1.);
  float br = (wavy-d);
  gl_FragColor = vec4(rgb(vec3(0.1*br,1.-br*.5,clamp(br*2.,0.,1.))),1.);
}
